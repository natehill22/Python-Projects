...
#Importing modules this will need access to
...
#Catches if the user clicks on the windows upper-right 'X'to ensure they want to close
def ask_quit(self):
...
def addToList(self):
    ...
    if not re.match(phoneValPattern, var_phone): #Format validations for phone number
        ...
        var_phone = ""
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #Requires fields to not be empty
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onUpdate(self):
    try:
        ...
        return
    #Updates will only be allowed for phone and emails. For name changes, records will need to be deleted and re-made.
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): #Ensures there is data present
        ...
        with conn:
            cur = conn.cursor()
            #Counts records to see if changes are already in database (to see if changes should be made)
